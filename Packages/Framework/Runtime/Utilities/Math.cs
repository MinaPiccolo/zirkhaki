    ...
    public static partial class UtilStatic
    {
        ...
        public static class Math
        {
            ...
            public static Vector3 RotatePointAroundPivot(Vector3 point, Vector3 pivot, float angle)
            {
                ...
                var rotatedX = Mathf.Cos(angle) * (point.x - pivot.x) - Mathf.Sin(angle) * (point.y - pivot.y) +
                               pivot.x;
                var rotatedY = Mathf.Sin(angle) * (point.x - pivot.x) + Mathf.Cos(angle) * (point.y - pivot.y) +
                               pivot.y;
